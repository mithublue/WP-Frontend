<template id="wpfront_new_form">
    <el-row>
        <el-col :sm="24">
            <div class="wpfront-edit-mode">
                <div class="mb20">
                ...
                <el-row :gutter="10">
                    <el-col :sm="16">
                        <div class="oh mb20 oh">
                            <div>
                                <h5 class="mb10 mt0"><?php _e( 'Form Title', 'wpfront' ); ?></h5>
                                <el-input type="text" v-model="form.post_title"></el-input>
                            </div>
                        </div>
                        <el-tabs type="border-card">
                            <el-tab-pane label="<?php _e( 'Form Builder', 'wpfront' ); ?>">
                                <div class="wpfront_form_builder_panel">
                                    ...
                                    <wpfront_new_form_root></wpfront_new_form_root>
                                </div>
                            </el-tab-pane>
                            <el-tab-pane label="<?php _e( 'Settings', 'wpfront' ); ?>">
			                    <?php $pages = get_pages();
			                    $neoform_pages = array();
                    ...
                    <el-col :sm="8">
                        <div class="mb20 action_panel mt30 oh">
                            <div class="alignleft">
                                <a href="javascript:" @click="update_form('draft')" class="button button-default"><?php _e( 'Save as Draft', 'wpfront' ); ?></a>
                                <a v-if="!form_id || form.post_status !== 'publish'" href="javascript:" @click="update_form('publish')" class="button button-primary"><?php _e( 'Publish', 'wpfront' ); ?></a>
                                <a v-if="form_id && form.post_status == 'publish'" href="javascript:" @click="update_form('publish')" class="button button-primary"><?php _e( 'Update', 'wpfront' ); ?></a>
                                <a @click="delete_form(1)" href="javascript:" class="el-button el-button--mini el-button--danger"><i class="el-icon el-icon-delete"></i> <?php _e('Move to Trash','wpfront'); ?></a>
                            </div>
                        </div>
                        <div>
                            <div class="wpfront_tabs">
                                <div class="tabs oh">
                                    <a href=""><?php _e( 'Elements Panel', 'wpfront' ); ?></a>
                                </div>
                                <div class="tab_cotnent p10">
                                    <div class="tab_pane">
                                        <div id="ui-wpfront_panel_ground-1">
                                            <div class="wpfront_element_tree">
	                                            <?php do_action('wpfront_before_element_tree' ); ?>
                                                <template v-if="!is_multistep">
                                                    <ul>
                                                        <template v-for="(row_data,r) in formdata.field_data">
                                                            <li class="ui-wpfront_row-1 wpfront_tree_placeholder" v-if="row_data.type == 'row'"
                                                                :target_row="r" :data-row_number="r"
                                                            >
                                                                <span class="wpfront_tree_placeholder_span pr wpfront_row_mover">
                                                                    <?php _e( 'Placeholder', 'wpfront' ); ?>
                                                                ...
                                                                <ul v-if="row_data.row_formdata.length && row_data == opened_tree_row">
                                                                    <template v-for="(field_data,c) in row_data.row_formdata">
                                                                        <li class="ui-wpfront_col-1 wpfront_tree_field"
                                                                            :target_col="c"
                                                                            :target_row="r"
                                                                        >
                                                                            <span class="wpfront_tree_field_span pr wpfront_col_mover">
                                                                                {{ field_data.s.label }}
                                                                            ...
                                                                            <div class="wpfront_tree_field_settings" v-if="opened_tree_col === field_data">
...
<script>
    ...
    var wpfrontend_new_form = {
        template: '#wpfront_new_form',
        data: function () {
            return {
                opened_tree_step: {},
                opened_tree_row: {},
        ...
        methods: {
            ...
            import_fields: function ( from, to ) {
                ...
                var self = this;
                setTimeout(function () {
                    wpfront_reset_sortable(self,self.is_multistep);
                },1);
            },
            ...
            fetchData: function () {
                ...
                if ( typeof  this.$route.params.form_type !== 'undefined' ) {
                    var _this = this;
                    /*if( typeof wpfront_form_type_data[this.$route.params.form_type] !== 'undefined') {*/
                ...
                //if edit form
                    ...
                    this.$store.dispatch('get_form',{id:this.$route.params.form_id, callback: function () {
                        return;
                        /*wpfront_reset_sortable_row(self, self.is_multistep);
                        setTimeout(function () {
                            wpfront_reset_sortable_field(self, self.is_multistep);
                        },1);*/
                    }});
                }
...
<template id="wpfront_row">
    <div class="ui-wpfront_row" @mouseover="make_focused_row(row_number)" @mouseout="defocus_row(row_number)" :target_row="row_number" :data-row_number="row_number" :class="{focused_row:is_focused_row}">
        <el-row style="text-align: center;">
            <wpfront_row_panel :target_row="row_number" v-if="is_focused_row && is_editing"></wpfront_row_panel>
            <template v-if="form_data.length">
                <template v-for="(field_data,k) in form_data">
                    <wpfront_input_template v-if="field_data.type == 'input' || field_data.inputType == 'text'"
                                             :field_data="field_data" :target_row="row_number" :target_col="k"></wpfront_input_template>
                </template>
            </template>
            <template v-else>
...
<template id="wpfront_new_form_root">
    <!--if -->
    <div id="ui-wpfront_builder_ground">

        <div :class="'wpfront_layout_' + form_settings.appearance_settings.s.layout_type">
            <el-form ref="form" label-width="120px" method="post">
                <div class="wpfront_db wpfront_center pr">
                    <template v-if="is_editing">
                        <a href="javascript:" @click="add_row()" class="wpfront_btn-add_row" title="<?php _e( 'Add Placeholder', 'wpfront' ); ?>">
                            <i class="dashicons dashicons-plus-alt"></i> <?php _e( 'Add Placeholder', 'wpfront' ); ?>
                        </a>
                    </template>
                    <a href="javascript:" @click="set_is_editing()" v-if="!is_editing" class="wpfront_btn-add_row" title="<?php _e( 'Edit', 'wpfront' ); ?>"><i class="dashicons dashicons-edit"></i> <?php _e( 'Edit', 'wpfront' ); ?></a>
                    <a href="javascript:" @click="unset_is_editing()" v-if="is_editing" class="wpfront_btn-add_row" title="<?php _e( 'Preview', 'wpfront' ); ?>"><i class="dashicons dashicons-visibility"></i> <?php _e( 'Preview', 'wpfront' ); ?></a>
                    <?php do_action('wpfront_formbuilder_header'); ?>
                </div>
                <?php do_action('wpfront_before_formbuilder' ); ?>
                <template v-if="!is_multistep">
                    <template v-for="(row_data,k) in field_data" v-if="render_field_list">
                        <wpfront_row v-if="row_data.type == 'row'" :row_number="k" :form_data="row_data.row_formdata"></wpfront_row>
                    </template>
                </template>
            </el-form>
        </div>
...
<script>
    ;document.addEventListener("DOMContentLoaded", function(event) {
        Vue.component('wpfront_new_form_root',{
            template: '#wpfront_new_form_root',
            data: function () {
                return {
...
<template id="wpfront_input_template">
    ...
    >
        <div @mouseover="make_focused_col(target_col)" @mouseout="defocus_col(target_col)"
            ...
            <wpfront_input_panel :field_data="field_data" v-if="is_focused_col() && is_editing"></wpfront_input_panel>
        </div>
    </el-col>
</template>
<script>
...
<template id="wpfront_input_panel">
    <div class="wpfront_input_panel">
        <a href="javascript:" @click="show_grids = true;set_edit_mode();" title="<?php _e( 'Resize', 'wpfront' ); ?>">
            <i class="dashicons dashicons-leftright"></i>
        </a>
        <a href="javascript:" @click="open_settings = !open_settings;toggle_edit_mode();" title="<?php _e( 'Settings', 'wpfront' ); ?>">
        ...
        <a href="javascript:" @click="remove_field()" title="<?php _e( 'Remove', 'wpfront' ); ?>">
            <i class="dashicons dashicons-trash"></i>
        </a>
        <a href="javascript:" class="wpfront_col_mover" title="<?php _e( 'Move Field'); ?>"><i class="dashicons dashicons-move"></i></a>
        <div v-if="open_settings" class="wpfront-field-settings-panel">
            <div class="wpfront_center mb5">
                <a href="javascript:" class="wpfront_btn-flat" @click="open_settings = false;unset_edit_mode();"><?php _e( 'Save', 'wpfront' ); ?></a>
                <a href="javascript:" class="wpfront_btn-flat" @click="cancel_editing();open_settings = false;unset_edit_mode();"><?php _e( 'Cancel', 'wpfront' ); ?></a>
            </div>
            <el-tabs type="border-card">
        ...
        <div class="wpfront_grid_panel" v-if="show_grids">
...
<script>
    ;document.addEventListener("DOMContentLoaded", function(event) {
        Vue.component('wpfront_input_panel',{
            template: '#wpfront_input_panel',
            props: ['field_data'],
            data: function () {
                return {
                    show_grids: false,
            ...
            methods: {
                ...
                toggle_edit_mode: function () {
                    if( this.open_settings ) {
                        ...
                        this.temp_col_formdata = wpfront_reset_fields(this.col_formdata.s);
                    } else {
                ...
                cancel_editing: function () {
                    if( Object.keys(this.temp_col_formdata).length ) {
                        this.col_formdata.s = wpfront_reset_fields( this.temp_col_formdata );
                    }
                },
...
<template id="wpfront_row_panel">
    <div class="row-panel pr">
        <a href="javascript:" @click="show_field_list();set_edit_mode();" v-if="!is_show_field_list"><i class="dashicons dashicons-plus-alt2"></i> <?php _e( 'Add Field', 'wpfront' ); ?></a>
        <a href="javascript:" @click="hide_field_list();unset_edit_mode();" v-if="is_show_field_list"><i class="dashicons dashicons-no-alt"></i> <?php _e( 'Close Panel', 'wpfront' ); ?></a>
        <a href="javascript:" @click="add_row(target_row)" title="<?php _e( 'Add Placeholder', 'wpfront' ); ?>"><i class="dashicons dashicons-plus-alt"></i> <?php _e( 'Add Placeholder', 'wpfront' ); ?></a>
        <a href="javascript:" @click="clone_row(target_row)" title="<?php _e( 'Clone Placeholder', 'wpfront' ); ?>"><i class="dashicons dashicons-welcome-add-page"></i></a>
        <a href="javascript:" @click="copy_row()" title="<?php _e( 'Copy Placeholder', 'wpfront' ); ?>">
            <i class="dashicons dashicons-admin-page"></i>
        </a>
        <a href="javascript:" v-if="copied_row" @click="paste_row()"><i class="dashicons dashicons-category"></i> <?php _e( 'Paste Placeholder', 'wpfront' ); ?></a>
        <a href="javascript:" v-if="copied_col" @click="paste_col()"><i class="dashicons dashicons-feedback"></i><?php _e( 'Paste Field', 'wpfront' ); ?></a>
        <a href="javascript:" @click="remove_row(target_row)" title="<?php _e( 'Remove Placeholder', 'wpfront' ); ?>"><i class="dashicons dashicons-trash"></i></a>
        <a href="javascript:" class="wpfront_row_mover" title="<?php _e( 'Move Placeholder'); ?>" title="<?php _e( 'Sort', 'wpfront' ); ?>"><i class="dashicons dashicons-move"></i></a>
        <wpfront_field_list :target_row="target_row"></wpfront_field_list>
    </div>
</template>
<script>
    ;document.addEventListener("DOMContentLoaded", function(event) {
        Vue.component('wpfront_row_panel',{
            template:'#wpfront_row_panel',
            props:['target_row'],
            data: function () {
                return {
                    open_menu: false,
...
<template id="wpfront_field_list">
...
<script>
    ;document.addEventListener("DOMContentLoaded", function(event) {
        Vue.component('wpfront_field_list',{
            template: '#wpfront_field_list',
            props: ['target_row'],
            data: function () {
                return {
